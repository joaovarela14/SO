...
function leitura_outputs() {
    ...
    while read -r line; do
        ...
        if [[ -v dicionario["$diretorio"] ]]; then
            ...
            tamanho_atual="${dicionario[$diretorio]}"
            diferenca=$((tamanho_atual - tamanho))
            dicionario["$diretorio"]=$diferenca
        else
            dicionario["$diretorio"]="$tamanho  NEW"
        fi
    ...
    while read -r line; do
        ...
        if [[ ! " ${diretorios_atuais[@]} " =~ " ${diretorio} " ]]; then
            dicionario["$diretorio"]="$tamanho  REMOVED"
        fi
